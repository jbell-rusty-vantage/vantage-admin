...
function Evidence({runId}:{runId:string}) {
 ...
 return <details><summary>Original evidence</summary>{list.error&&<p role="alert">Could not load evidence. <Button onClick={()=>void list.refetch()}>Retry</Button></p>}
 {list.data?.data.items.map(item=><p key={item.id}><Button onClick={()=>{setSnapshot(item.id);setOffset(null);}}>{item.tool?label(item.tool):'Retained evidence reference'} · {formatDateTime(item.retrieved_at)}</Button>{item.unavailable?' · Unavailable':''}</p>)}
...
export function AnalysisPanel({numberId,selectedRun,onSelect}:{numberId:string;selectedRun:string|null;onSelect:(id:string)=>void}) {
 ...
 return <section className="si-local-stack" aria-label="Analysis and Owner review"><h3>Analysis and Owner review</h3>
 {list.isPending&&<p role="status">Loading analyses…</p>}{list.error&&<p role="alert">Could not load analysis history. <Button onClick={()=>void list.refetch()}>Retry</Button></p>}
 ...
 {run&&<><p>{run.current?'Current analysis':'Earlier analysis'} · {label(run.status)} · Model: {run.model_version} · Prompt: {run.prompt_version}</p>
 ...
 <div className="si-local-filters"><Button disabled={!run.editable} onClick={()=>setCommand({action:'confirm_run'})}>Confirm analysis</Button><Button disabled={!run.output||!run.original_evidence_available} onClick={()=>setCommand({action:'original_evidence'})}>Reanalyze original evidence</Button><Button disabled={!run.output} onClick={()=>setCommand({action:'current_context'})}>Reanalyze current context</Button></div>
 {!run.original_evidence_available&&<p>Original evidence is unavailable for replay.</p>}
 ...
 {run.findings.map(f=><article key={f.id} className="si-local-stack"><h4>{label(f.assertion.kind)}</h4><p>{f.assertion.claim}</p><Badge>{f.review_state==='confirmed'?'Confirmed by you':f.review_state==='corrected'?'Corrected by you':label(f.review_state)}</Badge><p>Original model assertion · {run.model_version} · {label(String(f.assertion.basis??'Unknown basis'))}</p><p>AI-provided references; location and entailment have not been checked.</p><details><summary>Assertion references</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify(f.assertion.evidence,null,2)}</pre></details>
 ...
 <div className="si-local-filters"><Button disabled={!run.editable} onClick={()=>setCommand({action:'confirm_finding',findingId:f.id})}>Confirm finding</Button><Button disabled={!run.editable} onClick={()=>setCommand({action:'correct_finding',findingId:f.id})}>Correct finding</Button><Button disabled={!run.editable} onClick={()=>setCommand({action:'retract_finding',findingId:f.id})}>Retract finding</Button></div></article>)}
 {run.output?.next_step_suggestion&&<section><h4>Suggested next step</h4><p>{run.output.next_step_suggestion.description}</p><p>Suggestion only; applying it creates Owner-origin work.</p><Button disabled={!run.editable||!run.outreach} onClick={()=>setCommand({action:'apply_suggestion'})}>Apply suggestion</Button></section>}
 <section><h4>Owner changes and AI assessments</h4>{!run.instructions.length&&<p>No Owner instructions are recorded.</p>}{run.instructions.map(i=><article key={i.id}><p>{label(i.field)} · {i.actor} · {formatDateTime(i.happened_at)}</p><Badge>{i.assessment==='cannot_determine'?'Cannot determine':label(i.assessment)}</Badge><p>{i.reason}{i.stale?' An assessment exists for an earlier instruction version.':''}</p><details><summary>Owner change</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify({before:i.prior,after:i.current},null,2)}</pre></details></article>)}{!run.instructions_complete&&<p>Only the first 200 instructions are shown.</p>}</section>
