...
export function AnalysisPanel({numberId,selectedRun,onSelect}:{numberId:string;selectedRun:string|null;onSelect:(id:string)=>void}) {
 ...
 {run.processing_reason&&<p>{label(run.processing_reason)}</p>}
 ...
 <p>{run.output?.summary.overview??'No completed output.'}</p>
 ...
 <div className="si-local-filters"><TooltipCard title={copy.panel.confirmAnalysis} label={<Button disabled={!run.editable} onClick={()=>setCommand({action:'confirm_run'})}>{copy.panel.confirmAnalysis}</Button>}>{copy.commandExplain.confirm_analysis}</TooltipCard><Button disabled={!run.output||!run.original_evidence_available} onClick={()=>setCommand({action:'original_evidence'})}>{copy.panel.reanalyzeOriginal}</Button><Button disabled={!run.output} onClick={()=>setCommand({action:'current_context'})}>{copy.panel.reanalyzeCurrent}</Button></div>
 ...
 <div className="si-local-filters"><TooltipCard title={copy.panel.confirm} label={<Button disabled={!run.editable} onClick={()=>setCommand({action:'confirm_finding',findingId:f.id})}>{copy.panel.confirm}</Button>}>{copy.commandExplain.confirm_analysis}</TooltipCard><TooltipCard title={copy.panel.correct} label={<Button disabled={!run.editable} onClick={()=>setCommand({action:'correct_finding',findingId:f.id})}>{copy.panel.correct}</Button>}>{copy.commandExplain.correct}</TooltipCard><TooltipCard title={copy.panel.retract} label={<Button disabled={!run.editable} onClick={()=>setCommand({action:'retract_finding',findingId:f.id})}>{copy.panel.retract}</Button>}>{copy.commandExplain.retract}</TooltipCard></div></article>)}
 {run.output?.next_step_suggestion&&<section><h4>Suggested next step</h4><p>{run.output.next_step_suggestion.description}</p><p>{copy.panel.suggestionNote}</p><TooltipCard title={copy.panel.apply} label={<Button disabled={!run.editable||!run.outreach} onClick={()=>setCommand({action:'apply_suggestion'})}>{copy.panel.apply}</Button>}>{copy.commandExplain.apply_suggestion}</TooltipCard></section>}
