...
export const copy = {
  ...
  ui1: {
    ...
    /** UI1-OVERVIEW (UI-1 §4, addendum §6; COPY-UI1 §7). Placeholders are filled by the overview components. */
    overview: {
      /** `Updated {t}`: the TimeText prefix before the exact `as_of`. */
      updated: "Updated",
      period: {
        label: "Period",
        names: {
        ...
        /** No period chosen (`periods.activity.key` ≠ `periods.spend.key`): `Today · spend and outcomes: last 7 days`. */
        splitDefault: (activity: string, spend: string) => `${activity} · spend and outcomes: ${spend.toLowerCase()}`,
        note: "Calls aren't filtered by Priority.",
        from: "From",
        to: "To",
        apply: "Apply",
        customHint: "Dates are ET days.",
      },
      now: {
        title: "Now",
        needsReview: "Needs review",
        ...
        capture: { ok: "Capture healthy", attention: "Capture needs attention", broken: "Capture broken" } as Record<string, string>,
        captureUnknown: "Capture status not known yet.",
        captureLink: "Open Coverage",
        tileLabel: (name: string, count: string) => `${name}: ${count}. Open the list.`,
      },
      desk: {
        title: "Desk health",
        speedToLead: "Speed to lead",
        speedLine: (median: string, p90: string, leads: string, worked: string) => `median ${median} min · p90 ${p90} min in staffed time over ${leads} Leads (${worked} worked)`,
        stillWaiting: (n: string) => `${n} still waiting`,
        missedTarget: (n: string, target: string) => `${n} missed the ${target}-minute target`,
        callbacksKept: "Callbacks kept on time",
        keptLine: (kept: string, due: string, notKept: string, pending: string) => `${kept} of ${due} kept · ${notKept} missed · ${pending} not yet due`,
        keptTip: (contactUnknown: string, unreached: string) => `Kept includes ${contactUnknown} where contact is unknown and ${unreached} where the customer wasn't reached.`,
        overdueNow: (n: string) => `${n} overdue now`,
        missedReturned: "Missed calls returned",
        returnedLine: (onTime: string, episodes: string, late: string, open: string) => `${onTime} of ${episodes} returned within 15 staffed minutes · ${late} late · ${open} still open`,
        returnMedian: (n: string) => `median ${n} min to return`,
        flow: "Flow",
        flowIn: (n: string) => `In: ${n} new Outreach`,
        flowOutLabel: "Out",
        flowOut: {
          moved_to_quoted: "Moved to Quoted",
          booked_in_granot: "Booked in Granot",
          booked: "Booked",
          crm_bad_dead: "CRM bad/dead",
          owner_closed: "Closed by you",
        } as Record<string, string>,
        flowSegment: (label: string, n: string) => `${label} ${n}`,
        net: (n: string) => `Net ${n} active`,
        bandMoves: "Band moves",
        bandMovesRow: (band: number, into: string, out: string, duration: string) => `Band ${band}: ${into} in · ${out} out · median ${duration} in band`,
        unknownStart: (n: string) => `${n} without a known start`,
        flowFootnote: (excluded: string, repair: string) => `${excluded} moves from the one-time baseline or a policy change aren't counted; ${repair} came from a capture repair.`,
      },
      reps: {
        title: "Reps",
        columns: {
          ...
          talk: "Talk",
          attemptRate: "Attempt → conversation",
          bookingRate: "Booking rate",
          spend: "Lead spend",
          costPerBooking: "Cost per booking",
        },
        unpriced: (n: string) => `${n} unpriced`,
        recovered: (n: string) => `includes ${n} calls recovered by a capture repair`,
        unmapped: "Unmapped rep",
        unmappedNote: "Calls from extensions with no reviewed rep. Review them in RingCentral Accounts.",
        unassigned: "Unassigned",
        bySource: "By source",
        empty: "No rep activity in this period.",
        rateTip: (a: string, b: string) => `${a} of ${b}`,
        /** Proposals (not in COPY-UI1 §7): the talk cell, the expansion's band line and extension list, the sort button name, the expand button name. */
        talkMinutes: (n: string) => `${n} min`,
        openBands: "Open by band",
        openBand: (band: number, n: string) => `Band ${band}: ${n}`,
        extensions: "Extensions",
        sortBy: (column: string) => `Sort by ${column}`,
        expand: (name: string) => `Show details for ${name}`,
      },
      spend: {
        title: "Lead spend",
        total: (total: string, leads: string) => `${total} over ${leads} Leads`,
        split: (rate: string, legacy: string, unpriced: string, zero: string) => `${rate} at rate-period prices · ${legacy} at legacy prices · ${unpriced} unpriced · ${zero} at $0`,
        bySource: (source: string, n: string, cpl: string, spend: string) => `${source} · ${n} × ${cpl} = ${spend}`,
        bySourceMixed: (source: string, n: string, spend: string) => `${source} · ${n} Leads · ${spend} (mixed rates)`,
        outcomes: (leads: string, quoted: string, granot: string, booked: string, bookings: string) => `${leads} Leads · ${quoted} Quoted · ${granot} Booked in Granot · ${booked} Booked · ${bookings} of ${leads} booked`,
        /** Proposals: the by-source heading and the outcomes heading. */
  ...
  /**
   * UI-2 (COPY-UI2, design gate pending). One namespace per stage, additive. Server-rendered strings (timeline titles and
   * descriptions, `*_label`, `*_word`) are still shown as sent; these are the rep- and Users-specific strings the spec leaves open.
   */
  ui2: {
    ...
    /** UI2-SCOPE (UI-2 §3): card line 7 for the viewer, and the rep's card actions. */
    scope: {
      yours: "Yours",
    ...
    followup: {
      actions: { complete_followup: "Complete", snooze_followup: "Snooze", patch_followup: "Change date" } as Record<string, string>,
      titles: { complete_followup: "Complete this follow-up", snooze_followup: "Snooze this follow-up", patch_followup: "Change the follow-up date" } as Record<string, string>,
      dateLabel: { snooze_followup: "Snooze until", patch_followup: "New date" } as Record<string, string>,
      dateHint: "Up to 60 days from now.",
      noteLabel: "Note",
      noteHint: "Required. The Owner sees this note in the record's timeline.",
      notePlaceholder: "What happened, or why the date moves",
      send: "Send",
      sending: "Sending…",
      cancel: "Cancel",
      close: "Close",
      done: "Saved. The Owner sees your change.",
      errors: {
        tooFar: "Pick a date within 60 days.",
        forbidden: "You can't change this follow-up. Ask the Owner.",
        stale: "This follow-up changed since you opened it. Close and try again.",
        generic: "Couldn't save this. Try again.",
      },
      readOnly: {
    ...
    nudges: {
      ...
      alsoSent: "Also sent to your RingCentral",
      from: "Owner",
      skeletonLabel: "Loading messages from the Owner",
    },
    /** UI2-OVERVIEW (UI-2 §6): the rep's own row next to the anonymous team median (E23). */
    overview: {
    ...
    /** UI2-PHONE (UI-2 §7): the 390 px sheets and controls. */
    phone: {
      filters: "Filters",
      filtersCount: (n: number) => `Filters (${n})`,
    ...
    guide: {
      ...
      bandsIntro: "A band is the strongest reason a record needs a next step. Band 1 is the most urgent.",
      followupsBody: "On a follow-up assigned to you, you can Complete it, Snooze it or Change its date, up to 60 days ahead. Each change needs a short note. Follow-ups another rep owns, or that you only promised, are read-only: ask the Owner.",
      ownerBody: "Every change you make appears in the record's timeline with your name and your note, so the Owner sees what you did.",
      messagesBody: "When the Owner sends you a message about a record, it shows on that record's Work tab and was also sent to your RingCentral. You can't reply here.",
    },
    /** UI2-USERS (UI-2 §8): the Owner's Users tab in the Operations Registry. */
    users: {
      tab: "Users",
      title: "Users",
      intro: "Who can sign in to the admin, and what they can reach. Reps see only their own Sales Intelligence records.",
      ...
      roleHint: { owner: "Everything, including users.", admin: "Operations pages; no Sales Intelligence.", rep: "Their own Sales Intelligence records only." } as Record<string, string>,
      active: "Active",
      ...
      form: {
        addTitle: "Add a user",
        editTitle: (email: string) => `Edit ${email}`,
        email: "Email",
        role: "Role",
        agent: "Agent",
        agentHint: "Required for a rep. Only active Agents not linked to another active rep are listed.",
        agentPlaceholder: "Choose an Agent",
        noAgentsFree: "Every active Agent is already linked to an active rep.",
        access: "How they sign in",
        accessPassword: "Set a password now",
        accessInvite: "Send an invite link",
        password: "Password",
        passwordPolicy: "At least 10 characters, up to 72 bytes.",
        activeLabel: "Active",
        save: "Save",
        create: "Add user",
        cancel: "Cancel",
      ...
      setPassword: {
        title: (email: string) => `Set a password for ${email}`,
        body: "This signs them out everywhere. Tell them the new password yourself.",
        confirm: "Set password",
        done: (email: string) => `Password set for ${email}. They were signed out everywhere.`,
      },
      deactivate: {
        title: (email: string) => `Deactivate ${email}?`,
        body: "They're signed out at their next request and can't sign in again until you reactivate them in Edit.",
        confirm: "Deactivate",
        done: (email: string) => `${email} is deactivated.`,
      },
      inviteResult: {
        sent: (email: string, t: string) => `Invite emailed to ${email}. It expires ${t}.`,
        notEmailed: (reason: string) => `Not emailed: ${reason}`,
        reason: {
          not_configured: "no mail sender is set up. Copy the link and send it yourself.",
          failed: "the mail sender refused it. Copy the link and send it yourself.",
          unreachable: "the mail sender couldn't be reached. Copy the link and send it yourself.",
        } as Record<string, string>,
        expires: (t: string) => `The link expires ${t}.`,
        copy: "Copy link",
        copied: "Copied",
        linkLabel: "Invite link",
      },
      errors: {
        agent_required: "A rep needs a linked Agent.",
        agent_taken: "That Agent is already linked to another active rep.",
        agent_inactive: "That Agent is inactive. Choose an active Agent.",
        email_taken: "A user with this email already exists.",
        last_owner: "This is the last active Owner. Add another Owner before you change or deactivate this one.",
        invalid_input: "Check the fields and try again.",
        forbidden: "Only the Owner can manage users.",
        not_found: "This user no longer exists. Refresh the list.",
        unauthorized: "Your session ended. Sign in again.",
        user_inactive: "This user is deactivated. Reactivate them in Edit first.",
        generic: "Couldn't save this. Try again.",
        not_configured: "Invite links aren't set up on this admin yet (ADMIN_PUBLIC_BASE_URL). Set a password instead.",
        agent_check_unavailable: "Couldn't check the Agent right now. Try again in a minute.",
        agent_not_allowed: "Only a rep is linked to an Agent.",
        user_changed: "Someone else changed this user. Refresh the list and try again.",
      } as Record<string, string>,
      fieldErrors: { email: "Enter a valid email.", password: "At least 10 characters, up to 72 bytes.", agent: "Choose an Agent for a rep." } as Record<string, string>,
      empty: "No users yet.",
      loadError: "Couldn't load users.",
      errorCode: "Error code",
      tryAgain: "Try again",
      actionsFor: (email: string) => `Actions for ${email}`,
      created: (email: string) => `${email} added.`,
      saved: (email: string) => `Saved ${email}.`,
      inviteAfterCreate: (sentence: string) => `The user was added, but the invite wasn't sent: ${sentence}`,
      agentsLoadError: "Couldn't load the Agent list.",
      agentInactive: (name: string) => `${name} (inactive)`,
      inviteResultTitle: "Invite link",
      done: "Done",
      tableLabel: "Admin users",
    },
    /** UI2-USERS: the public accept-invite page (`/accept-invite#token=…`). */
    acceptInvite: {
      title: "Set your password",
      intro: "Choose a password for your Vantage admin account.",
      password: "Password",
      confirm: "Password again",
      policy: "At least 10 characters, up to 72 bytes.",
      submit: "Set password",
      submitting: "Setting…",
      success: "Password set. Sign in with your email and new password.",
      signIn: "Go to sign in",
      states: {
        malformed: "This invite link isn't complete. Open the link from your invite again, or ask the Owner for a new one.",
        invalid: "This invite link was already used or has expired. Ask the Owner for a new one.",
        weak: "Use at least 10 characters, up to 72 bytes.",
        mismatch: "The two passwords don't match.",
        generic: "Couldn't set your password. Try again.",
      },
    },
    /** Gallery section titles for the UI-2 components (dev only). */
    gallery: {
      sections: {
        "rep-shell": "Rep shell · view bar, header, not-available page",
        "rep-card": "Rep card · line 7 and actions",
        "rep-followup": "Rep follow-up actions",
        "rep-messages": "Messages from the Owner",
        "rep-overview": "Rep Overview · You and Team median",
        "rep-phone": "Phone sheets (390 px)",
        users: "Users tab",
