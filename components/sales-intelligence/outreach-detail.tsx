...
export function OutreachDetail({record}:{record:Outreach}) {
 ...
 return <section className="si-local-stack"><h3>Outreach · {label(record.state)}</h3>
 <OwnershipSplit overallOwner={record.assignment.agent}/>
 {record.lead_display&&<p>{record.lead_display.name??'Name not observed'} · Job Number {record.lead_display.job_no??'not observed'} · {record.lead_display.source_company??'Source Company not observed'}</p>}
 ...
 {record.latest_number_call&&<p>Latest activity on this number: {record.latest_number_call.direction} · {record.latest_number_call.provider_result??'Provider outcome unknown'} · {record.latest_number_call.contact_type==='human_conversation'?'Human conversation':record.latest_number_call.contact_type==='voicemail'?'Voicemail—speaker unknown':'Contact type unknown'} · {formatDateTime(record.latest_number_call.happened_at)}</p>}
 <p>Last human contact: {formatDateTime(record.last_meaningful_contact_at)}</p>
 ...
 <h3>Follow-ups</h3>{record.followups.filter(f=>f.status==='open').map(f=><div className="si-local-stack" key={f.id}><FollowupCard followup={f} overallOwner={record.assignment.agent}/><div className="si-local-filters">{f.allowed_actions.filter(action=>commandLabels[action.action]).map(action=><Button key={action.action} disabled={!action.enabled} onClick={()=>setEditing({command:action.action,actionId:f.id})}>{commandLabels[action.action]}</Button>)}</div></div>)}
 {!record.followups.some(f=>f.status==='open')&&<p>No active follow-ups recorded.</p>}
 <details><summary>Completed and cancelled follow-ups</summary>{record.followups.filter(f=>f.status!=='open').map(f=><FollowupCard key={f.id} followup={f} overallOwner={record.assignment.agent}/>)}</details>
