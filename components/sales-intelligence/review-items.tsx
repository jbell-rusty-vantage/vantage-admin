...
export function ReviewItems({subjectKey}:{subjectKey:string}) {
 ...
 return <section className="si-local-stack"><h3>Needs review</h3>{list.isPending&&<p role="status">Loading review decisions…</p>}
 {list.error&&<p role="alert">Review decisions could not refresh. <Button onClick={()=>void list.refetch()}>Retry reviews</Button></p>}
 {list.isSuccess&&!rows.length&&<p>No review decisions recorded for this subject.</p>}
 ...
 {list.hasNextPage&&<Button disabled={list.isFetching} onClick={()=>void list.fetchNextPage()}>More review decisions</Button>}
