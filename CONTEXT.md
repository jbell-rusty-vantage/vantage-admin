...
**Manual tab (admin-local pointer):** shipped Owner-only `/manual`. Create a Form Lead or Call Lead via the existing public write routes (`POST /api/v1/form-leads`, `POST /api/v1/call-leads`) with `ingestion_origin` assigned as Vantage Admin. The same page hosts Connect Booking to Lead for a Leadless Booking (booking search + reused `ConnectLeadPanel`). Connect also remains on `/bookings` detail. Not Booking Lead Reconciliation. Map: [`.cursor/rules/project-organization.mdc`](.cursor/rules/project-organization.mdc). Domain words stay in the root glossary.

**Booking intake robustness (admin-local pointer):** BILA-01–BILA-03 shipped — `/intakes` Find the right customer searches any-known-contact and shows Form submitted vs Granot (shared helper in `form-lead-contacts.tsx`). Confirm may omit a Lead; unique high auto-attaches; otherwise Master Booked only. `/bookings` detail hosts Connect Booking to Lead for a Leadless Booking (Stored lead column + in-place search). `/manual` reuses the same panel. Not `/bookings/reconciliation`. Pack: [`../vantage-main-server/docs/booking-intake-lead-attachment/README.md`](../vantage-main-server/docs/booking-intake-lead-attachment/README.md). Domain words stay in the root glossary.
...
**Operational surfaces (admin-local pointer, not a glossary term):** shipped / live (OSE-01–05). `OperationalResourcePage` still composes the eight list routes. The detail `SidePanel` is destination-tabbed (`?panel=`). List rows have an identity cell, status chips, one sticky-right Actions cluster, and a selected-row highlight (`?record=`). The filter sidebar groups Find / Status / Attribution / Record fields (same URL keys; not Observational `FilterBar`). Not Daily View. Contract: [`../vantage-main-server/docs/operational-surfaces/README.md`](../vantage-main-server/docs/operational-surfaces/README.md). Domain words stay in the root glossary.
