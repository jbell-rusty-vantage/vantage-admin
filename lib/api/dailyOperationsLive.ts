  ...
  try {
    if (eventName === "snapshot") {
      ...
      return {
        board: {
          ...current,
          snapshot: normalizeDailyOperationsSnapshot(payload),
          lastGoodAt: payload.generated_at ?? current.lastGoodAt,
          error: null,
        },
        flashedTiles: [],
      };
    ...
    if (eventName === "metrics") {
      const payload = JSON.parse(rawData) as { metric_touches?: string[] };
      const touches = Array.isArray(payload.metric_touches) ? payload.metric_touches : [];
      const increments = sessionDeltaIncrements(touches);
      const flashedTiles = Object.keys(increments) as DailyOperationsTileId[];
      const sessionDeltas = { ...current.sessionDeltas };
      for (const tile of flashedTiles) {
      ...
      return {
        board: {
          ...
          snapshot: current.snapshot
            ? applyDailyOperationsMetricTouches(current.snapshot, touches)
            : current.snapshot,
          sessionDeltas,
          lastGoodAt: current.snapshot?.generated_at ?? current.lastGoodAt,
          error: null,
        },
        flashedTiles,
      };
